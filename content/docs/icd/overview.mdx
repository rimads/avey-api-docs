...
<Callout type="info">Classification may include both generic and complete/specific ICD‑10 codes when available.</Callout>
...
<Mermaid
  chart={`
  %%{init: { "sequence": { "actorMargin": 140, "messageMargin": 36, "diagramMarginY": 24, "noteMargin": 16, "boxMargin": 16, "mirrorActors": false }}}%%
...
sequenceDiagram
  ...
  else Classify to ICD-10
    Client->>Classify: POST /icd/classify (clinical note text)
    Classify-->>Client: icd10[string] (list of ICD-10 codes)
  end
`}
/>
