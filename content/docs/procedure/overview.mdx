...
sequenceDiagram
    participant Client
    participant API as Adjudication API
    participant Engine as Rule Engine
    
    Client->>API: POST /adjudicate
    Note right of Client: Procedures + Diagnoses + History
    
    API->>Engine: Evaluate Context
    
    par Compliance Checks
        Engine->>Engine: MUE (Daily & Lifetime Limits)
        Engine->>Engine: PTP (Bundling Validation)
    and Clinical Checks
        Engine->>Engine: PLE (Diagnosis Justification)
    end
    
    Engine-->>API: Violations
